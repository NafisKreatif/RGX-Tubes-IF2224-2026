...
std::string InstructionLine::toString(std::size_t lineNumber) const {
    ...
    if (opCode != InstructionOpCode::RET || !operand.empty()) {
        ss << ' ' << level << ' ' << operand;
    }
...
std::size_t IntermediateCode::emitLOD(int address, std::string comment) {
    return emitInstruction(InstructionOpCode::LOD, 0, std::to_string(address), std::move(comment));
}

std::size_t IntermediateCode::emitSTO(int address, std::string comment) {
    return emitInstruction(InstructionOpCode::STO, 0, std::to_string(address), std::move(comment));
}

std::size_t IntermediateCode::emitCAL(int line, std::string comment) {
    return emitInstruction(InstructionOpCode::CAL, 0, std::to_string(line), std::move(comment));
}
